<?php
/*
Plugin Name: 0_LBU_graph7daysboxe
Description: Affiche les dernières mesures de boxe de l'utilisateur sous forme de graphique.
Version: 1.0
Author: Le Bon Univers
*/

function graphique_boxe_shortcode()
{
    ...
    $query = $bdd->prepare("SELECT nombre_rounds_boxe, duree_boxe, day FROM performances WHERE user_id = :user_id ORDER BY day DESC LIMIT 7");
    ...
    foreach ($results as $result) {
        ...
        $data_rounds[] = $result['nombre_rounds_boxe'];
        $duration = explode(':', $result['duree_boxe']);

        // Vérification de la validité de la valeur de durée
        if (count($duration) == 3 && is_numeric($duration[0]) && is_numeric($duration[1])) {
            $minutes = ($duration[0] * 60) + $duration[1];
        } else {
            // Log the problematic value for further investigation
            error_log("Invalid duration value found in the database: " . $result['duree_boxe']);
            // Use the problematic value as is (the original behavior)
            $minutes = $result['duree_boxe'];
        }
    ...
    $data_rounds = array_reverse($data_rounds);
    $data_duree = array_reverse($data_duree);

    $html = '<div class="chart-container">';
    $html .= '<canvas id="graphique-boxe" class="graph"></canvas>';
    $html .= '</div>';
    $html .= '<style>';
    ...
    $html .= '</style>';
    $html .= '<script>';
    $html .= 'var ctx = document.getElementById("graphique-boxe").getContext("2d");';
    ...
    $html .= '        datasets: [';
    $html .= '            {';
    $html .= '                label: "Nombre de rounds de boxe",';
    $html .= '                data: ' . json_encode($data_rounds) . ',';
    ...
    $html .= '            },';
    $html .= '            {';
    $html .= '                label: "Durée de la séance de boxe (en min)",';
...
add_shortcode('plug_graph7daysboxe', 'graphique_boxe_shortcode');
