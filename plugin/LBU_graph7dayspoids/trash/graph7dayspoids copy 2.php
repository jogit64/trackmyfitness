...
function afficher_donnees_utilisateur_connecte()
{
    ...
    $stmt = $dbh->prepare("SELECT poids_kg, pourcentage_gras, nombre_imc, created_at FROM performances WHERE user_id = :user_id ORDER BY created_at DESC LIMIT 7");
    ...
    foreach ($resultats as $resultat) {
        ...
        arraypush($nombre_imc, $resultat->nombre_imc);
        array_push($labels, date("d/m/Y", strtotime($resultat->created_at)));
    }
    $labels = array_reverse($labels);
    echo "Vous êtes utilisateur" . get_current_user_id();
?>
    <div>
