                    ...
                    function(response) {
                        if (response.success) {
                            ...
                            window.location.href =
                                "https://monsport.lebonunivers.fr/wp-content/plugins/LBU_recordpoidsandreloadpage/plug_recordpoidsandreloadpage_c.php";
                        }
                    }
                );
...
function enregistrer_donnees_form()
{
    if (isset($_POST['form_data']) && is_user_logged_in()) {
        ...
        $stmt = $pdo->prepare("INSERT INTO performances (user_id, poids_kg, pourcentage_gras, nombre_imc, day) VALUES (:user_id, :poids_kg, :pourcentage_gras, :nombre_imc, NOW())");
