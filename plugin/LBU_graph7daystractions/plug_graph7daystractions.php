<?php
/*
Plugin Name: 0_LBU_graph7daystraction
Description: Affiche les dernières mesures de tractions de l'utilisateur sous forme de graphique.
Version: 1.0
Author: Le Bon Univers
*/

function graphique_traction_shortcode()
{
    ...
    $query = $bdd->prepare("SELECT series_tractions, quantite_tractions, total_tractions, day FROM performances WHERE user_id = :user_id ORDER BY day DESC LIMIT 7");
    ...
    foreach ($results as $result) {
        ...
        $data_series[] = $result['series_tractions'];
        $data_quantite[] = $result['quantite_tractions'];
        $data_total[] = $result['total_tractions'];
    }
    ...
    $html .= '<canvas id="graphique-traction" class="graph"></canvas>';
    $html .= '</div>';
    $html .= '<style>';
    ...
    $html .= '</style>';
    $html .= '<script>';
    $html .= 'var ctx = document.getElementById("graphique-traction").getContext("2d");';
    ...
    $html .= '        datasets: [';
    $html .= '            {';
    $html .= '                label: "Séries de tractions",';
    ...
    $html .= '            },';
    $html .= '            {';
    $html .= '                label: "Quantité de tractions",';
...
add_shortcode('plug_graph7daystractions', 'graphique_traction_shortcode');
