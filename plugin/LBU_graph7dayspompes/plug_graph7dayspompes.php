<?php
/*
Plugin Name: 0_LBU_graph7dayspompes
Description: Affiche les dernières mesures de pompes de l'utilisateur sous forme de graphique.
Version: 1.0
Author: Le Bon Univers
*/

function graphique_pompes_shortcode()
{
    ...
    $query = $bdd->prepare("SELECT series_pompes, quantite_pompes, total_pompes, day FROM performances WHERE user_id = :user_id ORDER BY day DESC LIMIT 7");
    ...
    foreach ($results as $result) {
        ...
        $data_series[] = $result['series_pompes'];
        $data_quantite[] = $result['quantite_pompes'];
        $data_total[] = $result['total_pompes'];
    }
    ...
    $html .= '<canvas id="graphique-pompes" class="graph"></canvas>';
    $html .= '</div>';
    $html .= '<style>';
    ...
    $html .= '</style>';
    $html .= '<script>';
    $html .= 'var ctx = document.getElementById("graphique-pompes").getContext("2d");';
...
add_shortcode('plug_graph7dayspompes', 'graphique_pompes_shortcode');
