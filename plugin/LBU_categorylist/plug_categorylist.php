...
add_action('elementor/widgets/widgets_registered', function () {
    class Elementor_Categories_Widget extends \Elementor\Widget_Base
    {
        ...
        protected function _register_controls()
        {
            ...
            $this->add_control(
                ...
                [
                    ...
                    'options' => [
                        ...
                        'disc' => __('Cercle plein', 'elementor-custom-widget'),
                        'circle' => __('Cercle vide', 'elementor-custom-widget'),
                        'square' => __('Carré', 'elementor-custom-widget'),
                        'decimal' => __('Numéro', 'elementor-custom-widget'),
                    ],
                    'selectors' => [
                        '{{WRAPPER}} .ma-classe-css-personnalisable' => 'list-style-type: {{VALUE}};',
                    ],
                ]
            );
            ...
            $this->add_control(
                ...
                [
                    ...
                    'default' => '•',
                    'options' => [
                        '•' => __('Puce ronde', 'elementor-custom-widget'),
                        '▪' => __('Puce carrée', 'elementor-custom-widget'),
                        '‣' => __('Puce triangulaire', 'elementor-custom-widget'),
                        '-' => __('Tiret', 'elementor-custom-widget'),
                    ],
                    'condition' => [
                        'icon_type' => 'bullet',
                    ],
            ...
            // Ajout d'un contrôle pour la taille des icônes
            $this->add_control(
                'icon_size',
                [
                    'label' => __(
                    ...
                    'selectors' => [
                        '{{WRAPPER}} .ma-classe-css-personnalisable li a .icon' => 'font-size: {{SIZE}}{{UNIT}};',
                    ],
                ]
            );

            // Ajout d'un contrôle pour la taille de la police
            $this->add_control(
                'font_size',
                [
                    'label' => __(
            ...
            $this->add_control(
                ...
                [
                    ...
                    'max' => 3,
                    'selectors' => [
                        '{{WRAPPER}} .custom-class' => 'line-height: {{VALUE}};',
                    ],
                ]
            );
        ...
        protected function render()
        {
            ...
            $font_size = 'font-size: ' . $settings['font_size']['size'] . $settings['font_size']['unit'] . ';';
            $line_height = 'line-height: ' . $settings['line_height'] . ';';
            $icon_size = 'font-size: ' . $settings['icon_size']['size'] . $settings['icon_size']['unit'] . ';';

            echo '<ul class="ma-classe-css-personnalisable" style="list-style-type:' . $settings['list_style'] . '; ' . $font_size . $line_height . '">';

            //echo '<ul class="ma-classe-css-personnalisable" style="list-style-type:' . $settings['list_style'] . '; ' . $font_size . '">';
            ...
            foreach (get_categories() as $category) {
                ...
                echo '<li class="ma-classe-css-personnalisable">';
                echo '<a href="' . $link . '" style="' . $font_size . '">';

                if ($settings['icon_type'] === 'icon' && !empty($settings['icon']['value'])) {
                    echo '<span class="' . $settings['icon']['value'] . '" style="margin-right: ' . $settings['icon_spacing']['size'] . $settings['icon_spacing']['unit'] . '; ' . $icon_size . '"></span>';
                } elseif ($settings['icon_type'] === 'bullet' && !empty($settings['bullet'])) {
                    echo '<span class="bullet" style="margin-right: ' . $settings['icon_spacing']['size'] . $settings['icon_spacing']['unit'] . '; ' . $font_size . '">' . $settings['bullet'] . '</span>';
                }
